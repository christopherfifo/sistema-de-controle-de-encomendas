...
import { Search, ChevronDown, ChevronUp, UserMinus, UserCheck, Users, Building, ShieldCheck, X } from "lucide-react";
...
export function GerenciarMoradoresContent({ moradores, unidades, condominioId, sindicoId }: GerenciarMoradoresContentProps) {
  ...
  const [tipoOperacao, setTipoOperacao] = useState<"bloquear" | "reativar">("bloquear");
  const [moradorAlvo, setMoradorAlvo] = useState<{ id: string; nome: string } | null>(null);
  ...
  const moradoresFiltrados = moradores.filter((m) => {
    ...
    const correspondeStatus = 
      filtroStatus === "ativos" ? m.ativo === true :
      filtroStatus === "bloqueados" ? m.ativo === false : true;

    return correspondePesquisa && correspondeBloco && correspondeUnidade && correspondeStatus;
  });

  const abrirModal = (tipo: "bloquear" | "reativar", id: string, nome: string) => {
  ...
  return (
    <div className="space-y-6 w-full max-w-5xl">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">Controle de Ocupação & Cadastro</h2>
        <p className="text-muted-foreground">Gerencie desligamentos, reativações e visualize núcleos familiares em tempo real.</p>
      </div>
        ...
        <CardContent className="pt-6 grid gap-4 sm:grid-cols-4">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={(e) => { setBlocoSelecionado(e.target.value); setUnidadeSelecionada(""); }}
            >
              <option value="">Todos os Blocos</option>
              {blocosDisponiveis.map((b) => (
                <option key={b} value={b}>Bloco / Torre {b}</option>
              ))}
            </select>
          </div>
      ...
      <div className="space-y-3">
        {moradoresFiltrados.map((m) => {
          ...
          return (
            <Card key={m.id_usuario} className={`overflow-hidden border transition-all duration-200 ${!m.ativo && "border-destructive/30 bg-destructive/[0.01]"}`}>
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={`mt-1 p-2 rounded-lg hidden sm:block ${m.ativo ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}>
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 font-bold rounded-full ${m.ativo ? "bg-green-100 text-green-700" : "bg-destructive/10 text-destructive"}`}>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">CPF: {maskCPF(m.cpf)} • E-mail: {m.email}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                  {m.ativo ? (
                    <Button
                      ...
                      onClick={(e) => { e.stopPropagation(); abrirModal("bloquear", m.id_usuario, m.nome_completo); }}
                    >
                    ...
                    <Button
                      ...
                      onClick={(e) => { e.stopPropagation(); abrirModal("reativar", m.id_usuario, m.nome_completo); }}
                    >
                ...
                <CardContent className="border-t bg-muted/10 p-4 space-y-4 animate-in slide-in-from-top-2 duration-200">
                  <div className="grid gap-4 sm:grid-cols-2 text-sm">
                    <div className="space-y-1">
                      <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider">Dados da Conta</p>
                      <p><strong>Telefone:</strong> {m.telefone || "Não informado"}</p>
                      <p><strong>Situação Cadastral:</strong> {m.ativo ? "✓ Liberado para as dependências" : "✕ Acesso cortado da portaria"}</p>
                      <p><strong>Data de Entrada:</strong> {new Date(m.data_criacao).toLocaleDateString("pt-BR")}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {m.unidades_residenciais.map((ur: any) => {
                      ...
                      return (
                        <div key={ur.unidade.id_unidade} className="bg-background border rounded-lg p-3 space-y-2">
                          <p className="text-xs font-bold text-primary">Residentes do Apartamento {ur.unidade.numero_unidade} (Bloco {ur.unidade.bloco_torre}):</p>
                              ...
                              {coabitantes.map((co: any) => (
                                <div key={co.usuario.id_usuario} className="flex justify-between items-center py-2 first:pt-0 last:pb-0">
                                  <div>
                                    ...
                                    <p className="text-[11px] text-muted-foreground">CPF: {maskCPF(co.usuario.cpf)}</p>
                                  </div>
                                  <p className="text-muted-foreground">{co.usuario.telefone}</p>
                                </div>
                              ))}
                            </div>
                          )}
                    ...
                    {m.unidades_residenciais.length === 0 && (
                      <p className="text-xs text-muted-foreground italic p-2 bg-background border rounded-lg">Este usuário não possui nenhuma unidade associada no momento. Reative-o para vinculá-lo a uma moradia.</p>
                    )}
                  </div>
                </CardContent>
              )}
      ...
      {modalAberto && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <Card className="w-full max-w-md shadow-2xl relative animate-in zoom-in-95 duration-200">
            <button onClick={() => setModalAberto(false)} className="absolute right-4 top-4 text-muted-foreground hover:text-foreground p-1 rounded-md">
              <X className="h-4 w-4" />
            </button>
            
            <CardHeader>
              <CardTitle className={`text-xl flex items-center gap-2 ${tipoOperacao === "bloquear" ? "text-destructive" : "text-green-600"}`}>
                <ShieldCheck className="h-5 w-5" /> {tipoOperacao === "bloquear" ? "Bloquear Morador" : "Reativar Cadastro"}
              </CardTitle>
              <CardDescription>
                {tipoOperacao === "bloquear" ? (
                  <>Esta ação cortará os acessos de <strong>{moradorAlvo?.nome}</strong> e limpará seus vínculos com os apartamentos.</>
                ) : (
                  <>Você está prestes a reativar a credencial de <strong>{moradorAlvo?.nome}</strong> para reingresso no condomínio.</>
                )}
              </CardDescription>
            </CardHeader>
            ...
            <CardContent>
              <form onSubmit={handleExecutarOperacao} className="space-y-4">
                ...
                {tipoOperacao === "reativar" && (
                  <div className="space-y-1.5">
                    <Label htmlFor="modal-unidade" className="font-semibold text-xs">Vincular à Unidade Habitacional</Label>
                    <select
                      id="modal-unidade"
                      className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                      ...
                      required
                    >
                      <option value="">-- Selecione o Apartamento de Destino --</option>
                      {unidades.map((u) => (
                ...
                <div className="space-y-2">
                  <Label htmlFor="token-input" className="text-sm font-semibold">
                ...
                <div className="flex gap-2 pt-2">
                  <Button 
                    type="submit" 
                    variant={tipoOperacao === "bloquear" ? "destructive" : "default"} 
                    className={`flex-1 ${tipoOperacao === "reativar" && "bg-green-600 hover:bg-green-700"}`}
                    disabled={isPending}
                  >
                    {isPending ? "Processando..." : tipoOperacao === "bloquear" ? "Confirmar Bloqueio" : "Confirmar Reativação"}
                  </Button>
                  <Button type="button" variant="outline" onClick={() => setModalAberto(false)} disabled={isPending}>
                    Cancelar
                  </Button>
                </div>
              </form>
