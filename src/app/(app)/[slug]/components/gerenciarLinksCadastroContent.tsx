...
export function GerenciarLinksCadastroContent({
  condominios,
}: GerenciarLinksCadastroContentProps) {
    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">
          Links de Cadastro de Moradores
        </h2>
        <p className="text-muted-foreground">
          Compartilhe os links abaixo para que os novos moradores realizem o
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="flex flex-col justify-between">
          <CardHeader>
            ...
            <CardDescription>
              Links fixos de cada condomínio sob sua gestão. Ideal para fixar no
              mural ou bio do condomínio.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {condominios.map((condo) => {
              ...
              return (
                <div
                  key={condo.id_condominio}
                  className="p-3 bg-muted/40 border rounded-xl space-y-2"
                >
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleCopiar(urlPermanente, condo.id_condominio)
                      }
                    >
                      {estáCopiado ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-amber-500" /> Gerar Link
              Personalizado
            </CardTitle>
            <CardDescription>
              Crie um link específico na hora, útil para campanhas de
              cadastramento ou rastreamento de origem.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleGerarLinkNaHora} className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="origem">
                  Origem/Identificador do Link (Opcional)
                </Label>
                <Input
                  id="origem"
            ...
            {linkGeradoNaHora && (
              <div className="mt-6 p-4 bg-amber-50/50 dark:bg-amber-950/10 border border-amber-200 rounded-xl space-y-2 animate-in fade-in-50 duration-300">
                ...
                <div className="flex gap-2">
                  <Input
                    value={linkGeradoNaHora}
                    readOnly
                    className="h-9 text-xs bg-background border-amber-200"
                  />
                  <Button
                    size="sm"
                    variant="outline"
                    ...
                    onClick={() =>
                      handleCopiar(linkGeradoNaHora, "gerado-na-hora")
                    }
                  >
                    {copiadoUrl === "gerado-na-hora" ? (
