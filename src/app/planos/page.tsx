...
const infoPlanos: Record<string, { descricao: string, icone: React.ElementType, cor: string, suporte: boolean }> = {
  "Plano Light": { 
    ...
    <div className="container mx-auto py-16 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-[90rem] mx-auto">
        {planos.map((plano) => {
          ...
          return (
            <Card key={plano.id_plano} className="group flex flex-col h-full border-2 border-muted hover:border-primary/50 transition-all duration-300 hover:shadow-xl">
              <CardHeader className="text-center pb-2">
                <div className={`mx-auto w-12 h-12 rounded-full bg-muted flex items-center justify-center mb-4 ${info.cor}`}>
                  <Icone className="w-6 h-6" />
                </div>
                <CardTitle className="text-xl font-bold">{plano.nome_plano}</CardTitle>
              ...
              <CardContent className="flex-grow">
                <div className="text-center mb-8">
                  <div className="flex items-center justify-center">
                    <span className="text-xs font-semibold text-muted-foreground self-start mt-2 mr-1">R$</span>
                    <span className="text-4xl font-extrabold tracking-tight">
                      {Number(plano.valor).toFixed(2).replace('.', ',')}
                    </span>
                    <span className="text-muted-foreground ml-1 self-end mb-2 text-xs">/mês</span>
                  </div>
                </div>
                <ul className="space-y-3">
                  ...
                  <li className="flex items-center gap-3">
                    <CheckCircle2 className="h-4 w-4 text-green-500 flex-shrink-0" />
                    <span className="text-sm font-medium">
                      {plano.nome_plano === "Plano Light" ? "Até 5 Administradores" : "Administradores Ilimitados"}
                    </span>
                  </li>
              ...
              <CardFooter className="pt-6">
                <Button asChild className="w-full h-10 text-sm font-semibold shadow-sm hover:shadow-md transition-all duration-300 group-hover:bg-primary group-hover:text-primary-foreground" variant="outline">
      ...
      <div className="mt-16 text-center max-w-2xl mx-auto">
        <p className="text-muted-foreground mb-4">
          Possui um condomínio com mais de {maxUnits} unidades ou necessidades específicas?
        </p>
        <a 
